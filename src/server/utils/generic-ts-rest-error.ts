...
function buildDuplicateValueMessage(detail?: string, constraint?: string): string {
  ...
  if (!match) {
    return `A record with a duplicate unique value already exists${formatConstraintHint(constraint)}.`;
  }
  ...
  return `A record already exists with ${pairs.join(', ')}.`;
}
  ...
  switch (error.code.toUpperCase()) {
      ...
      return {
        ...
        body: {
          message: error.column
            ? `The field ${humanizeIdentifier(error.column)} is required.`
    ...
    case '23503':
      return {
        status: 409,
        body: {
          message: `Cannot complete the operation due to an invalid related reference${formatConstraintHint(error.constraint)}.`,
          code: 'CONFLICT',
        },
      };
    case '23505':
    ...
    case '23514':
      return {
        status: 400,
        body: {
          message: `Cannot save because a data validation failed${formatConstraintHint(error.constraint)}.`,
          code: 'BAD_REQUEST',
        },
      };
    case '22001':
    ...
    case '22003':
      return {
        status: 400,
        body: {
          message: 'One of the numeric values is out of the allowed range.',
          code: 'BAD_REQUEST',
        },
      };
    case '22P02':
      return {
        status: 400,
        body: {
          message: 'One of the values has an invalid format.',
          code: 'BAD_REQUEST',
        },
      };
    default:
...
export function genericTsRestErrorResponse(
  ...
  if (isError(error)) {
    return {
      status: 500,
      body: { message: genericMsg, code: 'INTERNAL_ERROR' },
    };
  }
